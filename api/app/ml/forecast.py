...
total minus baseline), and rolls everything up into a timeline + per-junction peaks.
"""
...
from app.ml.features import (
    Event, FEATURE_COLUMNS, baseline_congestion, build_feature_row,
    clock_hour, phase_for, timeline_offsets
)

ARTIFACT_DIR = Path(__file__).resolve().parent.parent / "artifacts"
AFFECTED_DELTA = 8.0  # Min congestion points over baseline to consider junction affected
...
class Forecaster:
    """Singleton wrapper around trained ML models."""
    ...
    def __init__(self) -> None:
        ...
        if not cong_path.exists() or not delay_path.exists():
            raise FileNotFoundError("Model artifacts not found. Run python -m app.ml.train first.")
    ...
    def forecast(self, event: Event) -> dict:
        ...
        # Build feature matrix (junctions x timesteps)
        ...
        for j in JUNCTIONS:
            for m in offsets:
                ...
                baseline[k] = baseline_congestion(j.base_volume, hour, event.is_weekend, event.rain)
                k += 1

        pred_c = np.clip(self.model_congestion.predict(rows), 0, 100).reshape(n_j, n_t)
        pred_d = np.clip(self.model_delay.predict(rows), 0, None).reshape(n_j, n_t)
        ...
        for ti, m in enumerate(offsets):
            cong_col, delay_col, delta_col = pred_c[:, ti], pred_d[:, ti], delta[:, ti]
            ...
            timeline.append({
                ...
                "congestion": {jid: round(float(cong_col[ji]), 1) for ji, jid in enumerate(junction_ids)},
                "delta": {jid: round(float(delta_col[ji]), 1) for ji, jid in enumerate(junction_ids)},
                "delay": {jid: round(float(delay_col[ji]), 2) for ji, jid in enumerate(junction_ids)},
            })
        ...
        for ji, jid in enumerate(junction_ids):
            ...
            per_junction.append({
                "id": jid, "name": j.name, "lat": j.lat, "lng": j.lng,
        ...
        kpis = {
            ...
            "avgDelayAtPeak": round(float(np.mean([p["delay"] for p in affected])) if affected else 0.0, 1),
            "totalDelayAtPeak": round(float(timeline[peak_index]["totalDelay"]), 1),
        }

        return {
            "event": {
                "venueId": event.venue_id, "venueName": venue.name,
                "venueLat": venue.lat, "venueLng": venue.lng,
                "venueCapacity": venue.capacity, "eventType": event.event_type,
                "attendance": event.attendance, "attendanceRatio": round(event.attendance / venue.capacity, 2),
                "startHour": event.start_hour, "dow": event.dow,
                "isWeekend": bool(event.is_weekend), "isHoliday": event.is_holiday,
                "rain": event.rain, "tempC": event.temp_c, "durationMin": event.duration_min,
            },
            "timeline": timeline,
