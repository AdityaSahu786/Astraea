"""Train the Astraea forecasting models and export model artifacts."""
...
def train(n_events: int = 440, test_frac: float = 0.18, seed: int = 7) -> dict:
    ...
    # Split by EVENT to evaluate generalisation to unseen events
    ...
    for target in ("congestion", "delay"):
        ...
        print(f"[train] Fitting model for {target}...")
        model = _make_model()
        ...
        print(f"[train]   {target}: MAE={mae:.3f} | R2={r2:.4f}")

    # Scatter sample for validation
    ...
    print(f"[train] Successfully trained in {metrics['trained_seconds']}s -> Saved to {ARTIFACT_DIR}")
    return metrics
