"""Curated road network graph for central + arterial Bengaluru.

Uses NetworkX to build an undirected graph of major junctions and corridors.
Calculates free-flow travel time, road capacities, and Betweenness Centrality.
"""
...
class Junction:
    ...
    base_volume: int    # Baseline peak throughput (veh/hr)
...
def build_graph() -> nx.Graph:
    """Build NetworkX graph with node geometry, edge capacities, and betweenness centrality."""
    g = nx.Graph()
    for j in JUNCTIONS:
        g.add_node(j.id, name=j.name, lat=j.lat, lng=j.lng, lanes=j.lanes, base_volume=j.base_volume)
    ...
    for a, b, road, lanes in _EDGES:
        ...
        length = haversine_km(ja.lat, ja.lng, jb.lat, jb.lng)
        capacity = lanes * 850  # Saturation flow 850 veh/hr per lane
        ff_time = length / 35.0 * 60.0  # Urban arterial speed ~35 km/h
        g.add_edge(
            a, b, road=road, lanes=lanes, length_km=round(length, 3),
            capacity=capacity, ff_time_min=round(ff_time, 2)
        )

    # Compute Structural Importance via Betweenness Centrality
...
def graph_to_dict() -> dict:
    """Serialise graph nodes and edges for the API & Leaflet frontend."""
    g = build_graph()
    nodes = [
        {
            "id": nid, "name": d["name"], "lat": d["lat"], "lng": d["lng"],
            "lanes": d["lanes"], "baseVolume": d["base_volume"], "centrality": d["centrality"],
        }
        for nid, d in g.nodes(data=True)
    ]
    edges = [
        {
            "from": a, "to": b, "road": d["road"], "lanes": d["lanes"],
            "lengthKm": d["length_km"], "capacity": d["capacity"],
        }
