"""Astraea API — Predictive Event Traffic Command Center for Bengaluru.

FastAPI service exposing forecasting, recommendation engine, road network graph,
venues, and post-event learning (predicted-vs-actual) endpoints.
"""
...
@app.get("/")
def root():
    return {"service": "Astraea API", "status": "ok", "docs": "/docs"}
...
@app.get("/api/venues")
def venues():
    return [{**v.to_dict(), "typicalEvents": list(v.typical_events)} for v in VENUES]


@app.get("/api/event-types")
def event_types():
    return [{"key": et.key, "label": et.label} for et in EVENT_TYPES.values()]
...
def events():
    ...
    for h in HISTORICAL_EVENTS:
        ...
        out.append({
            ...
            "venueId": e.venue_id, "venueName": venue.name,
            "eventType": e.event_type, "eventTypeLabel": EVENT_TYPES[e.event_type].label,
            "attendance": e.attendance, "startHour": e.start_hour,
            "durationMin": e.duration_min, "rain": e.rain,
        })
    return out
...
def replay(event_id: str):
    ...
    for p in forecast["perJunction"]:
        ...
        if not act: continue
        pred_c, act_c = p["peakCongestion"], act["congestion"]
        errs.append(abs(pred_c - act_c))
        comparison.append({
            "id": p["id"], "name": p["name"], "predicted": pred_c,
            "actual": act_c, "error": round(pred_c - act_c, 1), "delta": p["delta"],
        })
    ...
    return {
        ...
        "comparison": comparison, "accuracy": accuracy,
    }
